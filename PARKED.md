...
- 2026-07-21 · **WebView2 browser pane is unreliable**: worked fully once (round-29 proof), then silent async failures — queue drains, no error, no window; also one collapsed-window (~276x45) case. Ruled out: profile lock from orphaned webviews, missing feature flag. Root cause not found within budget. `gmux browse` therefore defaults to the SYSTEM browser (hardened explorer path) with the pane behind `--pane` · wake: a dedicated debugging round with the GUI run from a console (live stderr) and WebView2 event logging.
